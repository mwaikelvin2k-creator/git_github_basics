...
def count_specific_word(words):
    new_word = words.lower().replace('.', '').replace(',','').split()
    for word in new_word:
        print(new_word.count(word))
        break

# Identify most common word
from collections import Counter
def identify_most_common_word(text):
    new_text = text.lower().replace('.', '').replace(',','').split()
    Common_word = Counter(new_text)
    return Common_word.most_common(1)[0]

# Calculate the average word length
def calculate_average_word_length(wor):
    clean_text = wor.lower().replace('.','').replace(',','').split()
    sum_clean = sum(len(clean) for clean in clean_text)
    avg_len = sum_clean / len(clean_text)
    if clean_text != 0 or sum_clean != 0:
        print(avg_len)
    else:
        ValueError
...
def count_paragraphs(tex):
    paragraphs = tex.strip().split("\n\n")
...
def count_sentences(tx):
    sentences = re.split(r'[.!? ]', tx.strip())
    sentences = [s for s in sentences if s.strip()]  # remove empty parts
    print(len(sentences))
